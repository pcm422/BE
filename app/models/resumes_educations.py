...
class ResumeEducation(Base):
    ...
    resumes_id = Column(Integer, ForeignKey("resumes.id"), nullable=False)  # FK = resumes.id
    education_type = Column(SQLEnum(EducationTypeEnum, name="educationtype"), nullable=True)  # 학교 타입
    school_name = Column(String(255), nullable=True)  # 학교명
    education_status = Column(SQLEnum(EducationStatusEnum, name="educationstatus"), nullable=True)  # 학력 상태
