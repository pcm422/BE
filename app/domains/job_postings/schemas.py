...
class JobPostingCreateWithImage(BaseModel):
    ...
    def as_form(
        ...
        education: Optional[str] = Form(None, description="요구 학력 (가능한 값: none, high, college_2, college_4, graduate)"),
        ...
        payment_method: Optional[str] = Form(None, description="급여 지급 방식 (가능한 값: hourly, daily, weekly, monthly, yearly)"),
        job_category: Optional[str] = Form(None, description=f"직종 카테고리 (가능한 값: {', '.join([e.name for e in JobCategoryEnum])})"),
        work_duration: Optional[str] = Form(None, description=f"근무 기간 (가능한 값: {', '.join([e.name for e in WorkDurationEnum])})"),
