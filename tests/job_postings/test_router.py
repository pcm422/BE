...
async def test_job_posting_crud_flow(async_client):
    ...
    # 3. 로그인 및 토큰 획득
    login_data = {"email": company_data["email"], "password": company_data["password"]}
    ...
    job_posting_data = {
        "title": "테스트 공고",
        "recruit_period_start": str(date.today()),
        "recruit_period_end": str(date.today() + timedelta(days=30)),
